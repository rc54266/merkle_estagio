            ...
            <div class="ty-vendor-communication-post__wrapper" id="messages_list_{$thread_id}">
                {if $messages}
                    {foreach $messages as $message}
                        <div
                            class="ty-vendor-communication-post__content vendor-communication-post-item {if $message.user_type == $auth.user_type}ty-vendor-communication-post__you {/if}ty-mtb-s">
                            {hook name="vendor_communication:items_list_row"}
                            <div class="ty-vendor-communication-post__date">
                                {$message.timestamp|date_format:"`$settings.Appearance.date_format`, `$settings.Appearance.time_format`"}
                            </div>
                            <div class="ty-vendor-communication-post__img">
                                {if $message.user_type == "V"}
                                    <a
                                        href="{"companies.products?company_id=`$message.vendor_info.logos.theme.company_id`"|fn_url}">
                            ...
                            <div class="ty-vendor-communication-post__info">
                                <div class="ty-vendor-communication-post {cycle values=", ty-vendor-communication-post_even"}"
                                    id="post_{$message.message_id}">
                                    <div class="ty-vendor-communication-post__message">{$message.message|nl2br nofilter}</div>
                                    <span class="ty-caret"> <span class="ty-caret-outer"></span> <span
                                            class="ty-caret-inner"></span></span>
                                </div>
                                <div class="ty-vendor-communication-post__author">
                                    {if $message.user_type == "C"}
                                        {__("vendor_communication.you")}
                                    {else}{$message.firstname}
                                    {$message.lastname}{/if}</div>
                            </div>
                            {/hook}
                        </div>
                    {/foreach}
